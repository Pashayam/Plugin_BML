...
public class BmlGtTypedHandler extends TypedHandlerDelegate {
    ...
    public Result beforeCharTyped(final char c, @NotNull final Project project, @NotNull Editor editor, @NotNull PsiFile editedFile, @NotNull final FileType fileType) {
        ...
        if (c == '>' && webEditorOptions != null && webEditorOptions.isAutomaticallyInsertClosingTag() && fileContainsBmlLanguage(editedFile)) {
            ...
            if (offset < editor.getDocument().getTextLength()) {
                elementAtCaret = element = provider.findElementAt(offset, BMLLanguage.class);

                if (element == null && offset > 0) {
                    // seems like a template language
                    // <xml_code><caret><outer_element>
                    elementAtCaret = element = provider.findElementAt(offset - 1, BMLLanguage.class);
                }
                if (!(element instanceof PsiWhiteSpace)) {
                    ...
                    if (element instanceof XmlToken) {
                        ...
                        if (tokenType == XmlTokenType.XML_START_TAG_START || tokenType == XmlTokenType.XML_END_TAG_START) {
                            if (offset > 0) {
                                PsiElement previousElement = provider.findElementAt(offset - 1, BMLLanguage.class);
                ...
                element = provider.findElementAt(editor.getDocument().getTextLength() - 1, BMLLanguage.class);
    ...
    public static boolean fileContainsBmlLanguage(@Nullable PsiFile editedFile) {
        if (editedFile == null) return false;
        if (editedFile.getLanguage() instanceof BMLLanguage) {
            return true;
        }
        ...
        if (provider.getBaseLanguage() instanceof BMLLanguage) {
            return true;
        }
        return provider instanceof TemplateLanguageFileViewProvider &&
                ((TemplateLanguageFileViewProvider)provider).getTemplateDataLanguage() instanceof BMLLanguage;
    }
