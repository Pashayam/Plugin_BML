...
public class BmlHighlightVisitor extends XmlElementVisitor implements HighlightVisitor, IdeValidationHost {
    private static final Logger LOG = Logger.getInstance(BmlHighlightVisitor.class);
    ...
    public void visitXmlProcessingInstruction(XmlProcessingInstruction processingInstruction) {
        ...
        if (parent instanceof XmlProlog && processingInstruction.getText().startsWith("<?xml")) {
            for(PsiElement e = PsiTreeUtil.prevLeaf(processingInstruction); e != null; e = PsiTreeUtil.prevLeaf(e)) {
                ...
                String description = BmlErrorMessages.message("bml.declaration.should.precede.all.document.content");
    ...
    public HighlightVisitor clone() {
        return new BmlHighlightVisitor();
    }
